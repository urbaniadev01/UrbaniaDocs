...
| Reutilización de refresh token robado | Captura de tráfico, dispositivo comprometido | El backend ya detecta esto (rotación + token_family, [[01-api/API_JWT_IMPLEMENTATION|JWT_IMPLEMENTATION]] §4.2); la app debe reaccionar correctamente al `403 DEVICE_NOT_RECOGNIZED` / `401 TOKEN_INVALID` forzando re-login completo |
...
Implementa exactamente lo definido como requisito del lado cliente en [[01-api/API_JWT_IMPLEMENTATION|JWT_IMPLEMENTATION]] §6.2:
...
- [ ] Limpiar caché local sensible relacionada a la sesión (ver [[APP_DATA_STRATEGY]] §3 — qué se borra vs qué sobrevive un logout)
...
> [!note] Requisito explícito de [[01-api/API_JWT_IMPLEMENTATION|JWT_IMPLEMENTATION]] §6.2
> "Invalidar tokens al detectar jailbreak/root."

- Paquete sugerido: `flutter_jailbreak_detection` o `safe_device` (verificar mantenimiento activo al momento de implementar — ver nota de versiones en [[APP_ARCHITECTURE]] §1).
...
> La app **nunca** captura, transmite, ni almacena el número completo de tarjeta, CVV, o fecha de expiración en código propio. Esto se delega 100% al SDK/checkout de la pasarela elegida (Wompi o PayU — ver [[APP_ARCHITECTURE]] §1, decisión pendiente de negocio).
...
- El contenido de notificaciones sensibles (ej. "tu pago fue aprobado", mensajes de chat) se trunca/genera con copy genérico en la notificación visible (lock screen) y el detalle completo solo se muestra dentro de la app ya autenticada/desbloqueada — ver [[APP_DATA_STRATEGY]] §5.
...
| Código (MASVS-CODE) | Obfuscation en release (§8), `flutter_lints` estrictos ([[APP_ARCHITECTURE]] §10) | ⬜ |
...
> Marcar cada fila al cerrar la sesión de seguridad correspondiente en [[APP_IMPLEMENTATION_PLAN]] — no al final del proyecto. Igual que el backend, "verificar antes de asumir" ([[01-api/API_AGENTS|AGENTS]] del API).
...
| Documento | Propósito |
|---|---|
| [[01-api/API_JWT_IMPLEMENTATION]] (API) | Fuente única de la seguridad del lado servidor |
| [[APP_API_INTEGRATION]] | Flujos de login/MFA/refresh tal como los ejecuta la app |
| [[APP_DATA_STRATEGY]] | Qué datos sensibles persisten localmente y por cuánto tiempo |
| [[APP_RELEASE_AND_OBSERVABILITY]] | Configuración de Crashlytics, App Check, firma de builds |
