...
Verificar que el entorno cumple con los requisitos del stack definido en **[[API_ARCHITECTURE]]** seccion 1 (Stack Tecnologico).
...
Crear la estructura de bounded contexts segun **[[API_ARCHITECTURE]]** seccion 2 (Arquitectura: Clean Architecture + DDD).
...
Editar `composer.json` segun **[[API_ARCHITECTURE]]** seccion 9 (Autoloading y Registro de Bounded Contexts).
...
> ramsey/uuid ^4.7 soporta UUID v7. Todas las claves primarias usan UUID v7 (ver [[API_DATABASE]]).
> ```php
...
Editar `config/database.php` segun **[[API_DATABASE]]** seccion 8 (Configuracion de Base de Datos) y **[[API_ARCHITECTURE]]** seccion 8.

### 4.2 Configurar Variables de Entorno

Copiar `.env.example` a `.env` y configurar segun **[[API_ARCHITECTURE]]** seccion 13 (Variables de Entorno Requeridas).
...
Requisitos del algoritmo y parametros definidos en **[[API_JWT_IMPLEMENTATION]]** seccion 3.1.
...
> Ver **[[API_JWT_IMPLEMENTATION]]** sección 3.1 para detalles completos del algoritmo RS256.
...
Basado en **[[API_DATABASE]]** secciones 2.1-2.5. **Orden de creación obligatorio** (por dependencias de foreign keys):
...
El esquema exacto de columnas, tipos, indices y constraints esta definido en **[[API_DATABASE]]** (secciones 2.1-2.5). Reglas de nomenclatura en **[[API_DATABASE]]** seccion "Convenciones de Nomenclatura".

Reglas criticas:
- Toda migracion DEBE implementar `down()` (reversible)
- Claves primarias: UUID v7
- Convenciones de indices y constraints segun **[[API_DATABASE]]**
...
Segun **[[API_ARCHITECTURE]]** seccion 4 (Reglas de Dependencia), el orden es:
...
Crear en `src/Auth/Domain/`. Tipos de componentes y ubicaciones definidos en **[[API_ARCHITECTURE]]** seccion 2 (Arquitectura) y seccion 6 (Mapeo Eloquent a Entidad de Dominio).

Reglas:
- Sin dependencias de Laravel ni paquetes de terceros
- Entidades puras con logica de negocio
- ValueObjects inmutables
- Excepciones especificas de dominio (ver **[[API_ARCHITECTURE]]** seccion 5)

### 8.3 Application Layer

Crear en `src/Auth/Application/`. UseCases orquestan entidades del Domain. DTOs deben ser `final readonly class` (PHP 8.2+). Ver **[[API_ARCHITECTURE]]** seccion 7 (Estado de UI con DTOs y Resources).
...
Reglas de mapeo en **[[API_ARCHITECTURE]]** seccion 6:
...
Registrar provider en `config/app.php` segun **[[API_ARCHITECTURE]]** seccion 9.
...
Crear `phpstan.neon` en raiz segun **[[API_ARCHITECTURE]]** seccion 12 (Configuracion de Herramientas de Calidad).
...
Crear `pint.json` en raiz segun **[[API_ARCHITECTURE]]** seccion 12.
...
Crear estructura de tests en `tests/` segun **[[API_ARCHITECTURE]]** seccion 10 (Testing Obligatorio). Espejo de `src/`:
...
> [!note] Nota
> La configuración completa de entorno de testing está en [[API_TESTING]] §2. Verificar que `DB_DATABASE=urbania_test` esté aislado de desarrollo/producción.
...
> [[API_IMPLEMENTATION_PLAN]]. Al completarla, actualizar [[API_SESSION_MANIFEST]].
...
|-----------|-----------|-----------|
| [[API_AGENTS]] | Mapa de navegacion, flujos de trabajo, reglas de oro | Raiz del proyecto |
| [[API_ARCHITECTURE]] | Stack tecnologico, DDD, reglas de dependencia, Docker, ADRs | Raiz del proyecto |
| [[API_DATABASE]] | Esquema PostgreSQL, convenciones de nomenclatura, migraciones | Raiz del proyecto |
| [[API_CONTRACT]] | Endpoints, request/response, autenticacion, errores, rate limiting | Raiz del proyecto |
| [[API_JWT_IMPLEMENTATION]] | Especificacion de seguridad JWT RS256 | Raiz del proyecto |
| [[API_TESTING]] | Especificaciones tecnicas de pruebas | Raiz del proyecto |
| [[API_IMPLEMENTATION_PLAN]] | Plan de sesiones incremental para el modulo Auth y posteriores | Raiz del proyecto |
| [[API_SESSION_MANIFEST]] | Estado actual del proyecto entre sesiones de desarrollo | Raiz del proyecto |
| [[API_DEVELOPMENT_GUIDE]] | Troubleshooting, decisiones tecnicas ad-hoc, comandos de verificacion rapida | Raiz del proyecto |
...
> [!note] Nota
> Esta seccion referencia el plan detallado en [[API_IMPLEMENTATION_PLAN]].
> El modulo Auth se implementa en 8 sesiones incrementales. Cada sesion produce
> codigo runnable y actualiza [[API_SESSION_MANIFEST]].
...
|--------|--------|----------|----------------------|-----------|
| 1 | Setup + Health Check | Infraestructura funcionando | [[API_SETUP_GUIDE]], [[API_ARCHITECTURE]], [[API_DATABASE]] | P0 |
| 2 | Domain Layer | Entidades, VO, excepciones, eventos | [[API_ARCHITECTURE]], [[API_DATABASE]], [[API_TESTING]] | P0 |
| 3 | Application + JWT | UseCases, DTOs, tokens RS256 | [[API_JWT_IMPLEMENTATION]], [[API_ARCHITECTURE]], [[API_TESTING]] | P0 |
| 4 | Infrastructure | Repositorios, mappers, Eloquent | [[API_DATABASE]], [[API_ARCHITECTURE]], [[API_TESTING]] | P0 |
| 5 | Presentation Basica | Login, Register, Logout, Me, Refresh | [[API_CONTRACT]], [[API_JWT_IMPLEMENTATION]], [[API_TESTING]] | P0 |
| 6 | Seguridad Avanzada | MFA, device fingerprint, sesiones, rotacion | [[API_JWT_IMPLEMENTATION]] (completo), [[API_TESTING]] | P0 |
| 7 | Password + Perfil | Forgot, reset, change, update, verify | [[API_CONTRACT]], [[API_DATABASE]], [[API_TESTING]] | P0 |
| 8 | Polish + CI/CD | Scribe, PHPStan, Pint, pipeline, ADRs | [[API_SETUP_GUIDE]], [[API_TESTING]] | P0 |
...
2. **Scope lock**: No agregar tareas de otra sesion durante la sesion actual.
   Si surge la necesidad, documentar como deuda tecnica en [[API_SESSION_MANIFEST]].
...
5. **Actualizar [[API_SESSION_MANIFEST]]**: Obligatorio al final de cada sesion.
...
6. **Contexto minimo**: Solo entregar al agente los documentos listados en
   "Documentos requeridos" de cada sesion en [[API_IMPLEMENTATION_PLAN]].
