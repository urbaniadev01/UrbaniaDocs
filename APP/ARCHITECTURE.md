...
> Este documento describe el cliente móvil que consume [[../API_CONTRACT|API_CONTRACT]] del backend Urbania API. No duplica las reglas de ese proyecto — donde aplique, se referencia en vez de copiar. La app tiene su propia arquitectura porque sus restricciones son distintas: corre en el dispositivo del usuario, debe funcionar con conectividad intermitente, y su superficie de ataque (binario distribuible, almacenamiento local) es diferente a la de un servidor.
...
| Almacenamiento seguro | **flutter_secure_storage** | Wrapper sobre Keychain (iOS) / Keystore (Android) — ver [[SECURITY]] §2 |
| Biometría | **local_auth** | Face ID / Touch ID / huella — ver [[SECURITY]] §3 |
...
| Analítica / Crash reporting | **Firebase Crashlytics** + **Firebase Analytics** | Mismo SDK que push (menor footprint), correlacionable con `trace_id` del API — ver [[SECURITY]] §7 |
...
| Testing | `flutter_test`, `mocktail`, `integration_test`, `patrol`, `golden_toolkit` | Ver [[TESTING]] |

> [!tip] Regla de versiones
> Igual que en el API ([[ARCHITECTURE]] del backend §1): siempre la última versión estable de cada paquete al momento de iniciar/retomar el proyecto. Si una versión nueva de Flutter o de un paquete introduce un breaking change, documentarlo en [[DEVELOPMENT_GUIDE]] antes de actualizar, no después.

> [!warning] Decisión pendiente de confirmar con negocio
> La pasarela de pagos (ver [[FEATURE_SCOPE]] §4) no se fija aquí porque es una decisión de negocio, no solo técnica. Candidatas razonables para Colombia/LatAm: **Wompi** (Bancolombia) o **PayU**. Ambas ofrecen SDK/checkout que evita que la app toque datos de tarjeta directamente (ver [[SECURITY]] §6, cumplimiento PCI-DSS SAQ A).

---

## 2. Arquitectura: Clean Architecture *Feature-First*

> A diferencia del backend (bounded contexts DDD estrictos, [[ARCHITECTURE]] del API §2), el cliente móvil usa **feature-first**: cada pantalla/funcionalidad es una carpeta autocontenida con sus tres capas. Es la adaptación pragmática estándar de Clean Architecture en Flutter — el dominio de una app cliente es mucho más delgado que el de un backend (no hay reglas de negocio complejas, en su mayoría se delega al API).
...
- `data/` implementa las interfaces de `domain/` usando Dio (red) y/o Drift (local), y es responsable de decidir **de dónde** viene el dato según la estrategia de [[DATA_STRATEGY]].
...
- Deep links (ej. abrir la app desde una notificación push de chat o desde el correo de verificación de email) se registran como rutas nombradas — ver [[FEATURE_SCOPE]] §6 y [[DATA_STRATEGY]] §5.
...
| DTOs nunca se exponen fuera de `data/` | Un cambio de contrato del API (ver [[API_INTEGRATION]]) rompe `presentation/` en cascada |
| `core/widgets/` no conoce ninguna feature | Los componentes globales (ver [[DESIGN_SYSTEM]]) deben ser reutilizables sin acoplarse a reservas, pagos, etc. |
...
> El patrón `sealed class` + `switch` exhaustivo de Dart 3 da exactamente las garantías de exhaustividad de un `Either` funcional, sin curva de aprendizaje adicional para el equipo ni dependencia externa. Si el equipo ya tiene experiencia previa con `fpdart`, es una sustitución válida — documentarlo como decisión ad-hoc en [[DEVELOPMENT_GUIDE]] si se cambia.

---

## 7. Mapeo DTO → Entity → UI

Igual que el backend usa Mappers para ir de Eloquent a Entidad de Dominio ([[ARCHITECTURE]] del API §6), el cliente usa el mismo patrón en `data/mappers/`:
...
- Los DTOs reflejan **exactamente** la forma JSON del API ([[API_INTEGRATION]]).
...
Configurado vía `flutter_flavorizr` o `--dart-define` (decisión a tomar en Sesión 1, ver [[SETUP_GUIDE]] §2). Cada flavor tiene su propio `applicationId`/`bundleId` para poder instalar `dev` y `prod` en el mismo dispositivo simultáneamente.
...
> [!tip] Nota
> Mismo criterio que el backend ([[ARCHITECTURE]] del API §14): cada ADR se crea cuando la decisión ya está aplicada en código, no como ejercicio teórico previo.
...
| [[API_INTEGRATION]] | Antes de tocar cualquier llamada a la API |
| [[SECURITY]] | Antes de tocar tokens, biometría, almacenamiento local |
| [[DATA_STRATEGY]] | Antes de decidir si un dato es offline-first u online-only |
| [[DESIGN_SYSTEM]] | Antes de crear cualquier widget/pantalla nueva |
| [[ACCESSIBILITY]] | Antes de cerrar cualquier feature de UI |
| [[FEATURE_SCOPE]] | Para entender el alcance de producto |
| [[TESTING]] | Al crear/modificar tests |
| [[SETUP_GUIDE]] | Al iniciar el proyecto |
| [[IMPLEMENTATION_PLAN]] | Antes de cualquier tarea, para saber en qué sesión está el proyecto |
