...
Configurar 3 flavors (`dev`, `staging`, `prod` — ver [[ARCHITECTURE]] §9) usando `flutter_flavorizr` o `--dart-define-from-file` con un `.env` por entorno. Decisión a tomar y documentar en [[DEVELOPMENT_GUIDE]] en la Sesión 1 según preferencia del equipo (flavorizr automatiza más, `--dart-define` tiene menos dependencias).
...
> Igual que [[ARCHITECTURE]] §1: última estable de cada paquete al momento de instalar. Si hay conflicto de versiones entre `firebase_*` y otro paquete, priorizar la resolución que mantenga Firebase actualizado (es el paquete con mayor superficie de breaking changes entre versiones mayores) y documentar la decisión en [[DEVELOPMENT_GUIDE]].
...
- [ ] Habilitar Analytics
- [ ] Habilitar **App Check** (ver [[SECURITY]] §8) — configurar Play Integrity API (Android) y App Attest (iOS)
...
> `localhost` solo funciona desde el emulador/simulador. Para probar desde un dispositivo físico contra el backend local, usar la IP de la red local o un túnel (`ngrok`) — documentar la IP/túnel vigente en [[SESSION_MANIFEST]] si cambia entre sesiones de desarrollo en equipo.
...
Ver [[SECURITY]] §5 para la estrategia. En setup, generar el hash SPKI del certificado actual del backend:
...
- `app_en.arb` desde el MVP (ver [[ARCHITECTURE]] §1).
...
| Documento | Propósito |
|---|---|
| [[ARCHITECTURE]] | Estructura y stack que este setup materializa |
| [[SECURITY]] | Configuración de pinning, App Check, secure storage |
| [[IMPLEMENTATION_PLAN]] | Sesión 1 detalla el slice vertical mínimo posterior a este setup |
