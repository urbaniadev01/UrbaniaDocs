...
| Pagos de administración | `PaymentsHomeScreen`, `PaymentDetailScreen`, `PaymentHistoryScreen`, `PaymentMethodsScreen` | Pendiente de diseño en backend — ver [[SECURITY]] §6 para restricciones de cumplimiento |
...
|---|---|---|
| Notificaciones push centralizadas | `NotificationsCenterScreen` | Ver [[DATA_STRATEGY]] §5 |
| Chat (con administración y/o entre vecinos, a definir alcance) | `ChatListScreen`, `ConversationScreen` | Ver [[DATA_STRATEGY]] §6 — requiere decisión de producto: ¿chat 1:1 con administración únicamente (más simple, menor riesgo de moderación) o también vecino-a-vecino (mayor engagement, requiere moderación/reporte de abuso)? |
| Modo sin conexión visible (banners, indicadores de sincronización) | Transversal | Ver [[DATA_STRATEGY]] §2, §4 |
...
> P3 existe para que el equipo no sienta presión de meter "funciones de engagement" dentro de las sesiones P0/P1 de [[IMPLEMENTATION_PLAN]]. Ninguna idea de P3 se implementa antes de que P0+P1 estén estables en producción con usuarios reales.

---

## 4. Pagos: Consideraciones de Producto

- Pasarela candidata: Wompi o PayU (decisión de negocio, ver [[ARCHITECTURE]] §1 ADR-M006).
...
- Conciliación: el estado de "pagado" mostrado en la app **siempre** viene de una consulta al backend (online-only, [[DATA_STRATEGY]] §1), nunca del callback del SDK de pago directamente — el backend es quien confirma con la pasarela vía webhook antes de marcar la cuenta como pagada.
...
| Documento | Propósito |
|---|---|
| [[ARCHITECTURE]] | Cómo se estructura cada feature en código |
| [[DATA_STRATEGY]] | Estrategia de datos detrás de cada feature de esta tabla |
| [[DESIGN_SYSTEM]] | Sistema visual con el que se construyen estas pantallas |
| [[IMPLEMENTATION_PLAN]] | Secuenciación real en sesiones de desarrollo |
