...
- Cada release a `prod` se etiqueta en git (`vX.Y.Z`) y se referencia en la nota de sesión correspondiente ([[SESSION_MANIFEST]]).
...
- Errores capturados explícitamente (ej. un `ApiFailure` con código `INTERNAL_ERROR`/`DATABASE_ERROR`) se reportan manualmente con contexto — ver formato exacto en [[SECURITY]] §7 (incluir `trace_id`, nunca PII).
...
- **Nunca** registrar PII como parámetro de evento (email, nombre, número de unidad) — usar el `user_id` (UUID) si se necesita correlación, consistente con la regla de [[SECURITY]] §7.

### 3.3 Performance Monitoring

- Firebase Performance Monitoring (o alternativa) para tiempo de arranque en frío, tiempo de respuesta de pantallas con datos de red, y trazas custom en operaciones críticas (login completo, sincronización de la cola de [[DATA_STRATEGY]] §4).

### 3.4 Correlación con el Backend

- Todo evento de error reportado por la app que se origina en una respuesta del API incluye su `trace_id` ([[API_INTEGRATION]] §7) — esto permite a un agente de backend ubicar el log exacto del lado servidor sin depender de timestamps aproximados, igual que el sistema de auditoría del API ([[../JWT_IMPLEMENTATION|JWT_IMPLEMENTATION]] §8).
...
> Solicitar solo los permisos estrictamente necesarios para cada feature en el momento en que se necesitan (permiso de cámara solo al abrir el flujo de QR/avatar, no al iniciar la app) — además de ser buena práctica de seguridad ([[SECURITY]] §10, MASVS-PLATFORM), reduce friction de revisión en ambas tiendas.
...
| Documento | Propósito |
|---|---|
| [[TESTING]] | Suite que debe pasar antes de cualquier build de release |
| [[SECURITY]] | Configuración de App Check, firma, obfuscation que se verifica en cada release |
| [[IMPLEMENTATION_PLAN]] | Sesión dedicada a CI/CD y polish final |
