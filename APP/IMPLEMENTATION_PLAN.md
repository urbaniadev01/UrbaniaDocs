...
2. **Auth es el único módulo de negocio con contrato de API real hoy.** El resto del plan (Reservas, Visitas, Pagos, Chat) depende de que `API_CONTRACT.md` del backend incorpore esos módulos — ver [[FEATURE_SCOPE]]. Este plan no asume fechas para eso; cuando el backend defina un módulo nuevo, se agrega una sesión nueva aquí siguiendo el mismo patrón.
3. **No bloquear UI esperando al backend**: las sesiones de infraestructura offline/sync (Sesión 7) se construyen contra un mock server basado en el contrato esperado (ver [[TESTING]] §4), para que el equipo móvil avance en paralelo al de backend.
...
[[SETUP_GUIDE]], [[ARCHITECTURE]], [[DESIGN_SYSTEM]]

### Tareas
- [ ] Crear proyecto Flutter con estructura de carpetas de [[ARCHITECTURE]] §2
- [ ] Configurar flavors (dev/staging/prod)
- [ ] Configurar tokens base de [[DESIGN_SYSTEM]] (`ColorScheme.fromSeed`, espaciado, tipografía)
...
- [ ] Decisión de flavors documentada en [[DEVELOPMENT_GUIDE]]
...
### Documentos requeridos
[[ARCHITECTURE]] §6-7, `API_CONTRACT.md` del backend §1
...
### Checklist de cierre
- [ ] Cobertura de `domain/` ≥90% (ver [[TESTING]] §5)
...
### Documentos requeridos
[[API_INTEGRATION]], [[SECURITY]] §2

### Tareas
- [ ] DTOs (`freezed`) de login, registro, refresh, me — alineados 1:1 con `API_CONTRACT.md` §1
- [ ] `AuthInterceptor`: inyección de `Authorization`, manejo de `User-Agent` (ver [[API_INTEGRATION]] §2 — **crítico**), silent refresh con mutex (§4)
- [ ] `SecureTokenStorage` (`flutter_secure_storage`)
- [ ] `AuthRepositoryImpl` implementando la interfaz de Sesión 2
- [ ] Mock server de Auth para tests de integración (ver [[TESTING]] §4)
...
### Documentos requeridos
[[DESIGN_SYSTEM]], [[ACCESSIBILITY]], [[API_INTEGRATION]] §3
...
- [ ] Mapeo completo de `error.code` → UI (tabla de [[API_INTEGRATION]] §3)
...
### Checklist de cierre
- [ ] Checklist de [[DESIGN_SYSTEM]] §6 aplicado a cada pantalla
- [ ] Checklist de [[ACCESSIBILITY]] §8 aplicado a cada pantalla
...
### Documentos requeridos
`JWT_IMPLEMENTATION.md` del backend §7, [[SECURITY]] §3
...
### Entregable
Checklist OWASP MASVS de [[SECURITY]] §10 completamente marcado.
...
### Documentos requeridos
[[TESTING]], [[RELEASE_AND_OBSERVABILITY]]

### Tareas
- [ ] Golden tests de `core/widgets/` y pantallas de Auth
- [ ] Flujos de integración (Patrol) de la lista de [[TESTING]] §3.4
...
### Entregable
Pipeline verde de extremo a extremo, cobertura cumpliendo [[TESTING]] §5.
...
### Documentos requeridos
[[ACCESSIBILITY]], [[RELEASE_AND_OBSERVABILITY]] §4
...
### Checklist de cierre
- [ ] [[SESSION_MANIFEST]] actualizado con versión exacta del release candidato
- [ ] Documentar en [[DEVELOPMENT_GUIDE]] cualquier issue encontrado en la auditoría de accesibilidad/seguridad final

---

## Sesiones 10+: Módulos de Negocio

Cada módulo nuevo (Reservas, Visitas, Pagos, Chat, PQRS — ver [[FEATURE_SCOPE]] §3) se planea como una sesión nueva **solo cuando el backend correspondiente esté "Diseñado" en `API_CONTRACT.md`**, siguiendo esta plantilla:

```
## Sesión N: <Módulo>
### Documentos requeridos
API_CONTRACT.md del backend §<sección del módulo>, [[FEATURE_SCOPE]], [[DATA_STRATEGY]]
### Tareas
...
Ver [[SESSION_MANIFEST]] — se actualiza al cierre de cada sesión con el mismo rigor que exige el backend ([[../AGENTS|AGENTS]] del API, Regla de Oro #11).

---

## Reglas de Oro para el Orchestrator

1. No iniciar una sesión de negocio (10+) sin confirmar que el contrato del backend correspondiente ya está "Diseñado", no solo "propuesto" en [[FEATURE_SCOPE]].
2. Verificar antes de asumir: si se retoma una sesión interrumpida, correr `flutter analyze && flutter test` antes de continuar, igual que el backend exige `composer test` ([[../AGENTS|AGENTS]] del API, "Regla Crítica: Verify Before Assume").
...
4. Si se encuentra una inconsistencia entre este plan y el contrato real del backend, se reporta e informa antes de seguir adelante (mismo principio que [[../AGENTS|AGENTS]] del API).
...
| Documento | Propósito |
|---|---|
| [[AGENTS]] | Navegación y reglas de oro |
| [[ARCHITECTURE]] | Stack y estructura |
| [[FEATURE_SCOPE]] | Qué se construye y en qué orden de negocio |
| [[SESSION_MANIFEST]] | Estado actual entre sesiones |
