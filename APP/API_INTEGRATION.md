...
priority: P0
module: mobile
tags: [api, integration, networking, mobile]
updated: 2026-06-18
---
...
> Ese header está deprecado en el backend ([[../JWT_IMPLEMENTATION|JWT_IMPLEMENTATION]] §4.3). El servidor calcula el fingerprint a partir de `User-Agent` + IP + `Accept-Language` + `X-Device-Name`. La app **nunca** debe intentar enviarlo ni replicar el cálculo localmente.
...
- [ ] **Reportar al equipo de backend** ([[../AGENTS|AGENTS]] del API: "si encuentras una inconsistencia... debes informarlo") que detectar el tipo de cliente vía regex de `User-Agent` es fragil para clientes no-browser. Sugerencia: un header explícito `X-Client-Type: mobile|web` controlado por el backend (no por el cliente, para evitar spoofing) sería más robusto a futuro. Hasta que eso exista, este workaround es obligatorio.

---

## 3. Mapeo de Errores del API → UI

Todo error del API llega en el formato único `{ error: { code, message, trace_id } }` ([[../API_CONTRACT|API_CONTRACT]] "Formato de Respuesta de Error"). La app **nunca** muestra `message` crudo del backend al usuario final salvo que ya esté pensado para UI — siempre se traduce por código:
...
| (sin conexión) | — | `NetworkFailure` — ver [[DATA_STRATEGY]] §2 (no es un error del API, es ausencia de red) |
...
4. Éxito → recibe `access_token` + `refresh_token` + `user` → persistir en secure storage ([[SECURITY]] §2) → navegar a Home.
...
- Todo error reportado a Crashlytics/Sentry ([[RELEASE_AND_OBSERVABILITY]] §3) debe incluir el `trace_id` de la respuesta como contexto adicional — permite a soporte/backend ubicar el log exacto en el servidor sin necesidad de timestamps aproximados.

---

## 8. Versionado de API

- Todas las rutas usan el prefijo `/api/v1` ([[../API_CONTRACT|API_CONTRACT]]). La app **no** asume la versión por configuración suelta: vive en `EnvConfig.apiBaseUrl` (ver [[ARCHITECTURE]] §9), de forma que un futuro `/api/v2` sea un cambio de configuración, no de código disperso.
- Si el backend introduce un endpoint nuevo, el flujo es: 1) actualizar este documento con el mapeo de UI, 2) crear el DTO correspondiente en `features/<feature>/data/dtos/`, 3) actualizar [[FEATURE_SCOPE]] si afecta el alcance de producto.
...
- [ ] Decidir si la respuesta se cachea localmente — ver [[DATA_STRATEGY]]
- [ ] Test de integración con mock server (ver [[TESTING]] §4)
...
| Documento | Propósito |
|---|---|
| [[ARCHITECTURE]] | Estructura del cliente, capas, manejo de errores tipado |
| [[SECURITY]] | Almacenamiento de tokens, biometría, certificate pinning |
| [[DATA_STRATEGY]] | Qué se cachea localmente y bajo qué estrategia |
| [[FEATURE_SCOPE]] | Alcance de producto y prioridad de cada flujo |
