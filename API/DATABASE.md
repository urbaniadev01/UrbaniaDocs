...
> **PostgreSQL corre exclusivamente dentro de Docker** (servicio `db`, ver [[ARCHITECTURE#11. Docker Compose (Desarrollo)]]). No se instala ni se ejecuta una instancia local/nativa de PostgreSQL en ningún entorno (desarrollo, testing o producción) — siempre vía contenedor.
...
> Ver [[ARCHITECTURE#13. Variables de Entorno Requeridas]] para la lista completa
...
| `mfa_backup_codes` | JSONB | NULLABLE | Array de 10 códigos de respaldo (hasheados con bcrypt) |
...
> [!note] Nota
> En Laravel migrations, usar `$table->enum('status', ['active', 'suspended', 'inactive'])` o crear el TYPE manualmente en PostgreSQL.
...
> [!note] Nota
> Implementar en migración de password_history. Ver [[SETUP_GUIDE]] Sección 7.2.
...
> [!note] Nota
> Implementar en migración de security_events. Ver [[SETUP_GUIDE]] Sección 7.2.
...
> Los roles son hardcoded en el enum `UserRole { ADMIN, USER }` ([[ARCHITECTURE]]).
> No requieren tabla de roles/permisos para el MVP. Los scopes JWT se derivan del rol.
> Ejemplo: ADMIN → acceso de lectura/escritura completo, USER → acceso de lectura limitado a sus propios recursos.
